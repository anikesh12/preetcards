...
import TemplateInfoBadge from '../components/TemplateInfoBadge';
...
const COLLAGE_LAYOUTS = [
  ...
  {
    value: 'customized-card',
    label: 'Customized Card',
    premium: true,
...
function CollagePreview({ layout }) {
  ...
  if (layout === 'customized-card') {
    return (
      <span className="collage-preview collage-preview--customized" aria-hidden="true">
        <span className="cp-block cp-polaroid" />
        <span className="cp-block cp-dot cp-dot-1" />
        <span className="cp-block cp-dot cp-dot-2" />
      </span>
    );
  }
  return (
...
export default function CreateCardPage() {
  ...
  const [pendingAdClientId, setPendingAdClientId] = useState(null);
  ...
  const handleSubmit = async (event) => {
    ...
    const selectedLayout = COLLAGE_LAYOUTS.find((o) => o.value === collageLayout);
    ...
    if (selectedLayout?.premium) {
      ...
      try {
        ...
        if (config?.rewardedAdUnitId) {
          ...
          setPendingAdClientId(config.rewardedAdClientId || null);
          setShowAdModal(true);
          setIsSubmitting(false);
          return;
        }
  ...
  return (
    <div className="create-card-page">
      <style>{`
        ...
        .collage-preview--customized .cp-polaroid {
          width: 30px;
          height: 34px;
          top: 4px;
          left: 17px;
        ...
        .collage-preview--customized .cp-dot {
          width: 6px;
          height: 6px;
          border-radius: 50%;
        }
        .collage-preview--customized .cp-dot-1 { top: 2px; left: 4px; background: #FF6F91; }
        .collage-preview--customized .cp-dot-2 { top: 8px; right: 4px; background: #FFC75F; }

        .collage-option-header {
          display: flex;
          align-items: center;
          justify-content: center;
          gap: 6px;
          ...
          <div className="collage-options" role="radiogroup" aria-label="Collage style">
            {COLLAGE_LAYOUTS.map((option) => (
              ...
              >
                ...
                <span className="collage-option-header">
                  <span className="collage-option-label">{option.label}</span>
                  {option.premium && <TemplateInfoBadge tooltipText={option.tooltip} />}
                </span>
              </label>
            ))}
          </div>
